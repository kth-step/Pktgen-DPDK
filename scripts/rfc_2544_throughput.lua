...
local function runTrial(pkt_size, rate, duration, count)
	...
	statTx = pktgen.portStats(sendport)[tonumber(sendport)].curr;
	statRx = pktgen.portStats(recvport)[tonumber(recvport)].curr;
...
local function runThroughputTest(pkt_size)
  ...
  local statTx = pktgen.portStats(sendport)[tonumber(sendport)].curr
